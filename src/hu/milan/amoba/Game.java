...
public class Game {
    ...
    public Game() {
        ...
        this.human = new Player(name, "X");
        this.computer = new Player("Gép", "O"); // gép automatikusan létrejön
        this.board = new Board(10, 10); // 10x10 tábla
    }
    ...
    public void start() {
        ...
        while (!board.isFull()) {
            System.out.println(current.getName() + " (" + current.getSymbol() + ") következik.");
            ...
            if (board.placeSymbol(row, col, current.getSymbol())) {
                ...
                if (checkWin(current.getSymbol())) {
                    ...
                    // Csak az ember győzelmét mentjük az adatbázisba
                    if (current == human) {
                        database.addWin(current.getName());
                    }
